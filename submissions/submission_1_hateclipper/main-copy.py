...
def get_arg_parser():
    ...
    #parser.add_argument('--fusion', default='align', choices=['align', 'align_shuffle', 'concat', 'cross', 'cross_nd', 'align_concat', 'attention_m'])
...
def main(args):
    ...
    collator = CustomCollator(args, dataset_train.fine_grained_labels, multilingual_tokenizer_path=None)
    ...
    wandb_logger.experiment.config.update(num_args)
    ...
    trainer = Trainer(
        gpus=args.gpus, max_epochs=args.max_epochs, max_steps=args.max_steps, gradient_clip_val=args.gradient_clip_val,
        ...
        limit_train_batches=args.limit_train_batches, limit_val_batches=args.limit_val_batches, deterministic=True
    )
...
if __name__ == '__main__':
    ...
    args["exp_name"] = f"{args['exp_name']}_{args['dataset']}_{args['sentence_encoder']}_{date.today()}"
